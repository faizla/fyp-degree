...
WordCloud = function(_parentElement){
...
WordCloud.prototype.initVis = function(){
    ...
    //Remove empty array []
    new_array=new_array.filter(e => e.length)

    //Merge all the array of string in one whole array
    var merge = [].concat.apply([], new_array);
    ...
    for(i=0;i<merge.length;i++) {
        if(!stopwords.includes(merge[i].toLowerCase())) {
            //valid word is push to new array
            res.push(merge[i])
        }
    }
    //empty previous array just to save ram space
    merge=[]
    ...
    //Initialize the count of whole words
    vis.countWords=res.length

    //count the frequency of words and store it to its own keywords
    res=res.reduce((total, letter) => {
    ...
    res = Object.keys(res).map(function(key) {
        return { text: key, size: res[key] };
    });

    //Initialize count of unique words
    vis.countUniqueWords = res.length
    ...
    res = res.filter(function(d){return d.size>relevanceSizeOfWC})
    ...
    scale.domain([0,d3.max(res,function(d) {return d.size})
      ]);
    ...
    var layout = d3.layout.cloud()
      .size([width, height])
      .words(res.map(function(d) { return {text:d.text, size:d.size}; }))
...
WordCloud.prototype.dummyDate = function(){
